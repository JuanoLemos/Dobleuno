...
>
> Última actualización: 2026-09-14 (v1.2.0) — el módulo 1 pasó de Astro a React y el portal se retiró (ADR-011).

## TL;DR

Dobleuno = **un solo sitio** con tabs/módulos. La landing pública introduce el club. La app logueada expone los módulos. Desde la Ola 11 el Codex también vive en React, con piel propia por ruta: no queda ningún stack de UI aparte.
...
|---|---|---|---|---|---|
| 1 | **Codex** (reglamento) | Codex | ✅ Ola 11 (v1.2.0) | React 18 + Dexie + Express API | 1796 reglas especiales y 751 items mágicos de tow.whfb.app. Búsqueda y paginado server-side, navegación por sección, impresión, lectura sin señal. |
...
**Decisión:** todo vive en `apps/web/` (React). El portal Astro se retiró en la Ola 11 (ADR-011): nunca se desplegó y el SEO que justificaba mantenerlo no existía.
...
**Trade-off:** se resigna el SEO. No se compensa con SSR: el contenido del Codex deriva de publicaciones de Games Workshop y lleva `noindex` a propósito (ADR-011).
...
**Aplicado a:** home cream (Cartógrafo), `/reglas` (Codex), armybuilder login (mapa oscuro + iluminado), cards con textura (iron para listas/units, leather para items). Desde la Ola 11 las piezas viven solo en `apps/web/public/brand/`.
...
| **11** | **Codex en React** ✅ | Arreglar el pipeline, portear el Codex a React, `noindex`, retirar el portal | 2-3 días |
| **12** | **Polish + deploy consolidado** | Un solo deploy (app + landing) | 2-3 días |
...
**Estado:** ✅ En React, en `/reglas` y `/items`. El corpus real son 1796 reglas, 751 items y 577
unidades, bajados del sitio en la Ola 11. Búsqueda y paginado server-side, facetas por sección y
por familia, ficha imprimible, cache en Dexie para leer sin señal.

**Pendientes:**
- Correr la traducción al español (el traductor está adaptado; el corpus hoy está en inglés).
- UI para las unidades: están en la base y en la API, pero no tienen pantalla en el Codex.
- Verificar la carga a Postgres (no hay Docker en la máquina del autor).

**Decisión:** público pero con `noindex`, y el portal Astro retirado. Ver
[ADR-011](arch/ADR-011-codex-react-noindex.md).
...
├── scripts/
│   ├── mirror-tow.ts         # Baja el sitio (manifest = sus sitemaps)
│   ├── parse-tow.ts          # JSON crudo → corpus normalizado
│   ├── validate-corpus.ts    # Gate de calidad — Ola 11
│   ├── translate-tow.ts      # Traducción con DeepSeek
│   ├── rules-sync.ts         # Orquestador
│   └── shortcuts/            # YA EXISTE
├── doc/
│   ├── MODULES.md            # ESTE DOC
